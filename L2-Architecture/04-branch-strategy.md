...
| 2026-03-09 | v0.8.0-dev | working tree | `uncommitted` | Chroma retrieval fallback 강화 | `query_related` 기반의 broad semantic fallback과 source-aware ranking을 추가해 KISA/금융보안원(FSI)/OWASP/NVD 문서를 더 안정적으로 정규화하고, Chroma summary에 다중 source 맥락을 반영 | `cd VSH_Project_MVP && python -m pytest tests/test_l2_retriever.py tests/test_l2_contracts.py tests/test_mock_analyzer.py tests/test_l2_verifiers.py tests/test_l2_patch_builder.py tests/test_e2e.py -q` -> `25 passed, 1 skipped` |
