...
from vsh.core.models import DependencyVuln, Finding, PackageRecord, ScanResult, Severity, VulnRecord
...
def _confidence_from_meta(finding: Finding) -> str:
    engine = (finding.meta or {}).get("engine", "")
    if engine == "semgrep":
        return "high"
    if engine in {"pattern", "typosquatting"}:
        return "medium"
    return "low"


def _risk_score_for_finding(finding: Finding) -> float:
    if finding.cvss is not None:
        return round(max(0.0, min(10.0, finding.cvss)), 1)
    base = SEVERITY_BASE.get(finding.severity, 5.0)
    if finding.reachability == "YES":
        base += 1.0
    elif finding.reachability == "NO":
        base -= 0.5
    return round(max(0.0, min(10.0, base)), 1)


def _risk_score_for_dependency(dep_vuln: DependencyVuln) -> float:
    base = SEVERITY_BASE.get(dep_vuln.severity, 5.0)
    if dep_vuln.vuln_id and dep_vuln.vuln_id.startswith("CVE-"):
        base += 0.5
    return round(max(0.0, min(10.0, base)), 1)
...
def normalize_finding(finding: Finding, index: int) -> VulnRecord:
    ...
    return VulnRecord(
        ...
        risk_score=_risk_score_for_finding(finding),
        confidence=_confidence_from_meta(finding),
...
def normalize_dep_vuln(dep_vuln: DependencyVuln, index: int = 1) -> PackageRecord:
    ...
    return PackageRecord(
        ...
        risk_score=_risk_score_for_dependency(dep_vuln),
        confidence="high" if advisory_source in {"CVE", "GHSA", "PYSEC"} else "medium",
        cvss_score=None,
        license=None,
        license_risk=False,
    ...
    for pkg in sbom.get("packages", []):
        ...
        records.append(
            PackageRecord(
                ...
                severity=None,
                risk_score=3.0 if _is_risky_license(pkg.get("license")) else 0.0,
                confidence="medium",
                cvss_score=None,
