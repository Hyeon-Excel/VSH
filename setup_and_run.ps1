...
$ApiLog = Join-Path $ProjectDir ".vsh_api.log"
...
try {
  ...
  $apiProc = Start-Process -FilePath "$PythonExe" -ArgumentList @("-m","uvicorn","vsh_api.main:app","--host","127.0.0.1","--port","3000") -WorkingDirectory $ProjectDir -PassThru -WindowStyle Minimized -RedirectStandardOutput $ApiLog -RedirectStandardError $ApiLog
  ...
  try {
    ...
    Write-Host "[VSH] WARNING: API health check failed. Desktop에서 상태 메시지를 확인하세요." -ForegroundColor Yellow
  }
