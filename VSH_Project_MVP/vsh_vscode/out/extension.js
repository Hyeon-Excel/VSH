...
function activate(context) {
    ...
    const showDetailsCmd = vscode.commands.registerCommand('vsh.showDetails', async (finding) => {
        showWebviewPanel(finding);
    });
    const markReviewedCmd = vscode.commands.registerCommand('vsh.markReviewed', async (finding) => {
        ...
        const filteredDiagnostics = currentDiagnostics.filter(d => d.code?.id !== finding.id);
...
async function analyzeFile(filePath) {
    const apiUrl = vscode.workspace.getConfiguration('vsh').get('apiUrl');
    try {
        const res = await axios_1.default.post(`${apiUrl}/scan/file`, { path: filePath });
...
async function analyzeProject(projectPath) {
    const apiUrl = vscode.workspace.getConfiguration('vsh').get('apiUrl');
    try {
        const res = await axios_1.default.post(`${apiUrl}/scan/project`, { path: projectPath });
        updateDiagnostics(res.data.findings);
    }
    catch (e) {
        vscode.window.showErrorMessage('VSH scan failed');
...
function updateDiagnostics(findings) {
    ...
    for (const f of findings) {
        ...
        const diagnostic = new vscode.Diagnostic(new vscode.Range(f.line - 1, 0, f.end_line - 1, 100), f.message, severity);
...
function showWebviewPanel(finding) {
    if (currentPanel) {
        ...
        currentPanel = vscode.window.createWebviewPanel('vshDetails', 'VSH Finding Details', vscode.ViewColumn.One, {});
        currentPanel.webview.html = getWebviewContent(finding);
...
function getWebviewContent(finding) {
