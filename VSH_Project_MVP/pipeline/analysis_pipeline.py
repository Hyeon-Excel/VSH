...
from models.vuln_record import VulnRecord
...
class AnalysisPipeline(BasePipeline):
    ...
    def run(self, file_path: str) -> dict:
        ...
        if not is_clean:
            ...
            # 7. VulnRecord 형식으로 LogRepo 저장
            for suggestion in fix_suggestions:
                matching_vuln = next(
                    (v for v in unique_findings
                     if f"{file_path}_{v.cwe_id}_{v.line_number}" == suggestion.issue_id),
                    None
                )
                if matching_vuln:
                    record = VulnRecord(
                        ...
                        vuln_type=matching_vuln.cwe_id,
                        cwe_id=matching_vuln.cwe_id,
                        cve_id=suggestion.cve_id,
                        severity=matching_vuln.severity,
                        cvss_score=suggestion.cvss_score,
                        reachability=suggestion.reachability,
                        kisa_ref=suggestion.kisa_ref or f"KISA — {matching_vuln.cwe_id}",
                        fss_ref=suggestion.fss_ref,
                        owasp_ref=suggestion.owasp_ref,
                        fix_suggestion=suggestion.fixed_code,
                    )
                    self.log_repo.save(record.to_dict())
