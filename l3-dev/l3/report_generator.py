...
적용기준 : KISA 시큐어코딩 가이드 | 금융보안원 체크리스트 | OWASP Top 10"""
    ...
    def _format_score(self, vuln_records: list[VulnRecord], package_records: list[PackageRecord]) -> str:
        EXCLUDE_PATHS = [
            "VSH-l1-l2-integration", "archive", "dashboard",
            "modules", "pipeline", "repository", "tests", "l3-dev"
        ]
        score_records = [
            r for r in vuln_records
            if not any(ex in (r.file_path or "") for ex in EXCLUDE_PATHS)
        ]
        counts = Counter(r.severity for r in score_records)
        critical = counts.get("CRITICAL", 0)
        high     = counts.get("HIGH", 0)
        medium   = counts.get("MEDIUM", 0)
        low      = counts.get("LOW", 0)

        deduction = min(25, critical*15) + min(20, high*3) + min(10, medium*1) + min(5, low*1)
        score = max(0, 100 - deduction)

        kisa_deduct = min(self.KISA_TOTAL, critical * 2 + high)
        kisa_pass = self.KISA_TOTAL - kisa_deduct
        kisa_pct  = round(kisa_pass / self.KISA_TOTAL * 100) if self.KISA_TOTAL > 0 else 0

        fss_deduct = min(self.FSS_TOTAL, critical * 2 + high)
        fss_pass = self.FSS_TOTAL - fss_deduct
